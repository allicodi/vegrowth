...
do_efficient_tmle <- function(
  ...
  while((mean_phi_0^2 + mean_phi_1^2 + mean_phi_ge_data^2) > tol & iter <= max_iter){
    ...
    target_wt <- (
      (data[[V_name]] / pi_1) *
        ((1 - data[[Y_name]]) / rho_bar_0_star) *
        (rho_0_star - rho_1_star) / (1 - rho_1_star)
    )
    target_data <- data.frame(
      G_scale = G_scale,
      target_wt = target_wt,
    ...
    target_fit <- suppressWarnings(glm(
      G_scale ~ offset(logit_mu_10_star_scale), 
      weight = target_wt,
      family = binomial(),
      data = target_data,
      start = c(0)
    ...
    phi_0_data <- phi_0(data, pi_0, rho_0_star, rho_bar_0_star, mu_01_star, psi_tilde_0_star, psi_0_star)
    phi_1_data <- phi_1(data, pi_1, pi_0, rho_0_star, rho_bar_0_star, rho_1_star, mu_11_star, mu_10_star, psi_tilde_1_star, psi_1_star)
    phi_ge_data <- phi_1_data - phi_0_data
